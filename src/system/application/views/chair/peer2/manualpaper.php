...
function ChangeCAToUser($email,$conf)
{
$query="select usrid from user where ( email = '".$email."');";
$result = mysql_query($query);
 while($row = mysql_fetch_array($result))
 {
   $usrid=$row['usrid'];
 //echo "email = $email and usrid = ".$row['usrid'];
 }
...
$result = mysql_query($sql);
 ...
 {
  echo "Error in Email address. may be clash";
 }
}

function ChangeUserToCA($email,$conf)
{
$query="select usrid from user where ( email = '".$email."');";
$result = mysql_query($query);
 while($row = mysql_fetch_array($result))
...
function ListRegisteredUser($confid)
{
  echo '<p></p><span id="userlist" > 
  	    <a href="#" onclick="ShowPapers('.$confid.',4)" >List All Attendee </a > | 
  	    <a href="#" onclick="UserManagement('.$confid.')" >Change Registered User Role </a > 
  		</span><p></p>';
 echo '<span id="detail">';
 ...
 $query="SELECT u.usrid,u.email,u.firstname,u.lastname FROM attendee a inner join cadmin ca on (a.usrid=ca.adminid && a.confid=$confid) inner join user u on (a.usrid=u.usrid)";
...
//SELECT * FROM attendee a inner join cadmin ca on (a.usrid=ca.adminid && a.confid=2) inner join user u on (a.usrid=u.usrid);
 ...
 while($row = mysql_fetch_array($result))
 {
  $ca[$i++]=$row['usrid'];
	...
	<td >'.$row['email'].'</td>
	<td >
	<select onchange="RoleChangeds(\''.$row['email'].'\',\''.$confid.'\',this.value)"
	<option  >Unprivileged User</option>
	<option selected="selected" >conference administrator</option>
	</select>
 ...
 $j=0;
 $query ="SELECT * FROM attendee a inner join user u on(a.usrid=u.usrid && a.confid=$confid";
 ...
 while($row = mysql_fetch_array($result))
 {
	...
	<td >
	<select  onchange="RoleChangeds(\''.$row['email'].'\',\''.$confid.'\',this.value)" >
	<option selected="selected" >Unprivileged User</option>
	<option >conference administrator</option>
	</select>
	</td>
 	</tr>';
 }
 echo '</table></span> <span id="breaknews"> </span> ';
}
...
/* ------------------------- [\]  Updated Code --------------------------*/
function ShowReviews($conf,$email)
{
 $query="SELECT u.email,f.descp,u.firstname,u.lastname FROM feedback f inner join user u on (f.usrid=u.usrid) inner join conference c on (f.confid=c.confid && c.confid=".$conf."&& c.startdate < now() && c.enddate < now()) inner join user ch on (c.chairid=ch.usrid && ch.email='".$email."');";
 $result = mysql_query($query);
 echo'<table style="width: auto"  class="style2" id="mytable">
	   <tr>
			<th>User(s)</th>
...
function ShowHistory($opt,$email)
{
 if ($opt==1)
 {
 $query="SELECT c.confid,datediff(now(),c.startdate)as remainday,c.shorttitle,c.fulltitle,c.startdate,c.enddate FROM conference c inner join user u on  (c.chairid=u.usrid && c.startdate < now() && c.enddate < now() && u.email='".$email."');";
 $result = mysql_query($query);
 echo'<table style="width: auto"  class="style2" id="mytable">
	   <tr>
...
{
$query="select r.firstname as rfname,r.lastname as rlname,p.abstract as papername, c.shorttitle as conferencename ,r.email as remail,datediff(c.subdeadline,now()) as deadline from paper p inner join conference c on (p.confid=c.confid && c.enddate > now()  && c.subdeadline > now() ) inner join user ch on (c.chairid=ch.usrid && ch.email='".$email."') inner join user r on (p.revid=r.usrid);";
$result = mysql_query($query);
echo'<table style="width: auto"  class="style2" id="mytable">
	   <tr>
			<th>Reviewer Name</th>
...
if (isset($_GET['usremail']))
{
	if (isset($_GET['role']))
	{
		...
		if ($role=="conference administrator")
		{
		 ChangeUserToCA($emails,$confs);
		} 
		else if ($role=="Unprivileged User")
		{
		 ChangeCAToUser($emails,$confs);
		} 
...
{
 if (isset($_GET['confid']) == true  && $_GET['registered'] == "true")
 {
  ListRegisteredUser($_GET['confid']);
 }
}
//////////////////////////////////////////////////////////////////////
